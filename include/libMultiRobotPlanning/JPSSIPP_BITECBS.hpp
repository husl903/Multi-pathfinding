...
public:
 ...
 public:
  JPSSIPP_BITECBS(Environment& environment, 
  std::vector<PlanResult<State, Action, Cost>>& ssolution, float w) : m_env(environment, ssolution), m_astar(m_env, w) {}
  ...
  bool search(const State& startState, const Action& waitAction,
    ...
    bool success = m_astar.search(JPSSIPPState(startState, interval, 0xf, 0, false), astarsolution);
...
//   };
  ...
  struct JPSSIPPEnvironment {
    JPSSIPPEnvironment(Environment& env,
        std::vector<PlanResult<State, Action, Cost>>& ssolution) : m_env(env), m_solution(ssolution) {}
...
//      std::cout << location.x << " " << location.y << "size " << iter.size() << " \n";
   ...
   private:
    Environment& m_env;
    std::vector<PlanResult<State, Action, Cost>>& m_solution;
 ...
 private:
  ...
  float m_w;
  AStarEpsilon<JPSSIPPState, JPSSIPPAction, Cost, JPSSIPPEnvironment, JPSSIPPStateHasher> m_astar;
};
