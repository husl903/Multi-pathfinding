...
template <typename State, typename Action, typename Cost, typename Environment,
          typename StateHasher = std::hash<State>, typename StateHasherOpen = std::hash<State> >
class JPSTAStar {
 public:
  JPSTAStar(Environment& environment) : m_env(environment) {}
  ...
  bool search(const State& startState,
    ...
    openSet_t openSet;
    std::unordered_map<State, fibHeapHandle_t, StateHasherOpen> stateToHeap;
