...
class SIPP {
 ...
 public:
  ...
  SIPP(Environment& environment, 
       std::vector<PlanResult<Location, Action, int> >& solution, float& w) : m_env(environment, solution), m_astar(m_env, w) {}
          ...
          for(size_t agent_id = 0; agent_id < m_cat.size(); agent_id++){
            ...
            if(current_time - 1 >= 0){
              if(current_time - 1 < m_cat[agent_id].states.size()){
                ...
                if(temp_s_p.x == neighbors[nei].state.state.x && temp_s_p.y == neighbors[nei].state.state.y
                  && temp_s.x == s.state.x && temp_s.y == s.state.y){
                  neighbors[nei].state.nc_cat++;
                }
              }
            }
...
//    std::unordered_map<Location, std::vector<edgeCollision>> m_edgeCollision;
 ...
 private:
  ...
  AStarEpsilon<SIPPState, SIPPAction, Cost, SIPPEnvironment, SIPPStateHasher> m_astar;
};
