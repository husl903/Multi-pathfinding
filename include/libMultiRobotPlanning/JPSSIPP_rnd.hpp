...
public:
 ...
 public:
  JPSSIPP(Environment& environment, RNDGameState& game_state) : m_env(environment, game_state), m_astar(m_env){}
  ...
  bool search(const State& startState, const Action& waitAction,
              PlanResult<State, Action, Cost>& solution, RNDGameState &state_game, Cost startTime = 0) {
...
//   };
  ...
  struct JPSSIPPEnvironment {
    JPSSIPPEnvironment(Environment& env, RNDGameState& game_state) : m_env(env), m_game_state(game_state){}
...
//          		  std::cout << flag_curr_i[i].size() << " " << flag_next_i[i].size() << "\n";
			  ...
			  m_game_state.board.grid.assign(s.state.grid.begin(), s.state.grid.end());
			  
			  m_game_state.board.need_update_index.clear();
			//   std::cout << " need_update_index " << s.state.need_update_index.size() << std::endl;
			  sort(s.state.need_update_index.begin(), s.state.need_update_index.end());
    		  m_game_state.board.need_update_index.assign(s.state.need_update_index.begin(), s.state.need_update_index.end());
    		//   m_game_state.resetLocalState(s.localstate);
    		  std::cout << m_game_state.board.rows << ", " << m_game_state.board.cols << " Test grid works \n";
			  for (int h = 0; h < m_game_state.board.rows; ++h) {
        			for (int w = 0; w < m_game_state.board.cols; ++w) {
          				std::cout << kCellTypeToElement[m_game_state.board.grid[h * m_game_state.board.cols + w] + 1].id;
            			// std::cout << kCellTypeToElement.at(state_game.board.grid[h * state_game.board.cols + w]).id;
        		    }
        		  std::cout << std::endl;
    		  } 
			  m_game_state.board.grid[s.state.y * m_game_state.board.cols + s.state.x] = 1;

			  m_game_state.board.agent_pos = 10;
     		...
     		while(true){
        		...
        		if(m_env.stateValid(current_successor.state) &&
    				...
    				if(successor_start != -1){
						...
						{
						//    std::cout << temp_s.state.x  << ", " << temp_s.state.y << " test ---" << std::endl;
						//    std::cout << kCellTypeToElement[m_game_state.board.grid[temp_s.state.y * m_game_state.board.cols + temp_s.state.x] + 1].id << std::endl;
						...
						if((m_game_state.board.grid[(temp_s.state.y)*m_game_state.board.cols + temp_s.state.x - 1] == 1 
						    && m_game_state.board.grid[(temp_s.state.y)*m_game_state.board.cols + temp_s.state.x - 2] ==2) || 
    					...
    					if(re_ac != -1){
							...
							current_successor.state.grid.assign(m_game_state.board.grid.begin(), m_game_state.board.grid.end());
							for( int w = s.state.x; w >= current_successor.state.x; w--){
								if(w > current_successor.state.x) current_successor.state.grid[current_successor.state.y * m_game_state.board.cols + w] = 1;
								if(w == current_successor.state.x) current_successor.state.grid[current_successor.state.y * m_game_state.board.cols + w] = 0;
							}							
...
/*    	             		if(m_env.isFCheck()){
		...
		step = 0;
     	if((dir & 0x02) && !flag_is_solution){ // the right direction
     		JPSSIPPState temp_s = s;
     		Cost current_cost_l = current_cost;
     		while(true){
     			m_env.num_generation++;
...
/*    	             		if(m_env.isFCheck()){
								...
								current_successor.state.grid.clear();
								current_successor.state.grid.assign(m_game_state.board.grid.begin(), m_game_state.board.grid.end());
								for( int w=s.state.x; w <= current_successor.state.x; w++){
									if(w < current_successor.state.x) current_successor.state.grid[current_successor.state.y * m_game_state.board.cols + w] = 1;
									if(w == current_successor.state.x) current_successor.state.grid[current_successor.state.y * m_game_state.board.cols + w] = 0;
            		...
            		while(true){
                		...
                		if(m_env.stateValid(current_successor.state) &&
            				...
            				if(successor_start != -1){
								...
								if((m_game_state.board.grid[current_successor.state.y * m_game_state.board.cols + current_successor.state.x] == 1 && 
								   m_game_state.board.grid[(current_successor.state.y + 1) * m_game_state.board.cols + current_successor.state.x] == 2) ||
								   (m_game_state.board.grid[current_successor.state.y * m_game_state.board.cols + current_successor.state.x] == 2  && 
								   m_game_state.board.grid[(current_successor.state.y + 1) * m_game_state.board.cols + current_successor.state.x] == 1)){
									re_start.push_back(startTime(m_lastGScore + current_cost_l + 1, Action::Up, 0x07, true));
								}
                		...
                		if(m_env.stateValid(current_successor.state) &&
            				...
            				if(successor_start != -1){
								...
								if((m_game_state.board.grid[current_successor.state.y * m_game_state.board.cols + current_successor.state.x] == 1 && 
								   ...
								   (m_game_state.board.grid[current_successor.state.y * m_game_state.board.cols + current_successor.state.x] == 2  && 
								   m_game_state.board.grid[(current_successor.state.y - 1) * m_game_state.board.cols + current_successor.state.x] == 1)){
									re_start.push_back(startTime(m_lastGScore + current_cost_l + 1, Action::Down, 0x0b, true));
								}								
            					std::sort(re_start.begin(), re_start.end()); //For the re-start direction, choose the minimum re-start time.
            					Cost re_ac = -1;
            					unsigned int current_dir = 0x00;
