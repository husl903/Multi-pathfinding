...
//            }
 ...
 public:
  ...
  int getEdgeIndex(int from, int to){
	  return (1+from)*m_dimx*m_dimy + to;
  }
...
//	  std::cout << " SetEdge--- " << s.x <<", " << s.y << ", "<< T <<"\n";
  ...
  void resetTemporalObstacle(){
    ...
    jpst_gm_->clear_temporal_obstacles();
  }

  void add_cat_obstacles(const Location& s){
    jpst_gm_->add_obstacle_cat(s.x, s.y);
  }
...
int main(int argc, char* argv[]) {
  ...
  //  if(num_agent_iter < 53) continue;
    ...
    if(solver == solver_jpst){
      ...
      timer_t1.stop();
      if(successJpst) std::cout << " Planning successful! time, " << timer_t1.elapsedSeconds() <<  ", " << inputFile << std::endl;
      else {
        std::cout << " Planning NOT successful! time, " << timer_t1.elapsedSeconds() << " ," << inputFile  << std::endl;
        break;
      }
    }
    ...
    if(solver == solver_jpsta){
      ...
      mapf.setBP(true);
      mapf.setCAT(true);
      std::cout << "Jpst-j-jtom2-, " << num_agent_iter << ", ";
    ...
    if(solver == solver_ancat){
      ...
      timer_t6.stop();
      if(successANoCAT) std::cout  << " Planning successful! time, " << timer_t6.elapsedSeconds() << ", "<< inputFile << std::endl;
    ...
    if(solver == solver_cancat){
      ...
      timer_t9.stop();
      if(successCANoCAT) std::cout  << " Planning successful! time, " << timer_t9.elapsedSeconds() << ", "<< inputFile << std::endl;
    ...
    if(solver == solver_jpstnobp){
      ...
      timer_t7.stop();
      if(successJpstNoBP) std::cout << " Planning successful! time, " << timer_t7.elapsedSeconds() << ", " << inputFile << std::endl;
  ...
  if (successSipp) {
    ...
    int cost = 0;
    int makespan = 0;
    for (const auto& s : solution_sipp) {
      cost += s.cost;
...
//   }
  ...
  //   std::cout << inputFile << " Planning successful! time here" << timer.elapsedSeconds() << std::endl;
  //   int cost = 0;
  //   int makespan = 0;
  //   for (const auto& s : solution_jpsta_path) {
  //     cost += s.cost;
  //     makespan = std::max<int>(makespan, s.cost);
  //   }
  ...
  //   std::cout << "Check Valid is here : \n";
  //   mapf.CheckValid(solution_jpsta);
  // } else {
  //   std::cout << inputFile << " Planning NOT successful!" << std::endl;
  // }
