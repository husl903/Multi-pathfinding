...
//whether needs const
  void getNeighbors(State& s,
    ...
    std::cout << "Current state "<< s.x <<", " << s.y << ", time " << s.time << ", hash, " << s.zorb_hash << ", size, " << s.grid.size()  << ",dir, " << s.dir << "----------------------------------"<< std::endl;
    // // if(s.time == 14 )
    // {
    ...
    if(s.dir & 0x04){
      ...
      if (index - 1 == state_game.board.agent_pos || state_game.board.agent_pos == kAgentPosExit) {
        ...
        if(state_game.board.zorb_hash != zorb_hash_temp_w) left.dir = 0xff;
          ...
          if(index - 2 >= 0 && index - 2 - m_dimy >=0){
            if(left.grid[index - 2] == 3 && left.grid[index - 2 - m_dimy] == 3) left.dir = left.dir | 0x08;   
          }
    ...
    if(s.dir & 0x08){
      ...
      if (index + 1 == state_game.board.agent_pos || state_game.board.agent_pos == kAgentPosExit) {
        ...
        if(state_game.board.zorb_hash != zorb_hash_temp_w) right.dir = 0xff;
          ...
          if(index + 2 < m_dimx*m_dimy && index + 2 -m_dimy > 0){
            if(right.grid[index + 2] == 3 && right.grid[index + 2 - m_dimy] == 3) right.dir = right.dir | 0x04;      
          }
...
int main(int argc, char* argv[]) {
    ...
    // filename="./levels/bd_01_1_test-02.txt";  
...
//    std::cout << board_str << std::endl;
    ...
    while(index_g < goals_loc.size()){
      ...
      timer.stop();
      getrusage(RUSAGE_SELF, &r_usage);
      if(success) std::cout <<filename <<  ", success, cost, " << solution.cost  << ",start, " << startX << ", " << startY << ", goal, " << goalX <<", " << goalY << ", memory, " << r_usage.ru_maxrss  << ", time, " <<  timer.elapsedSeconds() <<  ", Expansion, " << env.num_expand << ", generation, " << env.num_generated << ",num_action," << state_game.num_apply_action <<std::endl;    
      else std::cout << filename <<  ", not success, "<< ",start, " << startX << ", " << startY << ", goal, " << goalX <<", " << goalY << ", memory, " << r_usage.ru_maxrss  << ", time, " <<  timer.elapsedSeconds() <<  ", Expansion, " << env.num_expand << ", generation, " << env.num_generated << ",num_action," << state_game.num_apply_action <<std::endl;    
