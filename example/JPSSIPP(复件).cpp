...
int main(int argc, char* argv[]) {
  ...
  std::fstream res_sta("Berlin_scen_1_c", std::ios::app);
  std::fstream res_Good("good_Berlin_secn_1_c", std::ios::app);
...
//		  std::cout<< ob.x - 1 << " " << ob.y - 1 <<std::endl;
 ...
 typedef JPSSIPP<State, State, Action, int, Environment> sipp_t;
  ...
  for (size_t i = 0; i < goals.size(); ++i) {
    ...
    Environment env(dimx, dimy, obstacles, jumppoint, goals[i]);
    sipp_t sipp(env);
